...
impl RustaceanRepository{
    ...
    pub async fn find(c: &mut AsyncPgConnection, id: i32) -> QueryResult<Rustacean>{
        rustacean::table.find(id).get_result(c).await
    }
    pub async fn find_multiple(c: &mut AsyncPgConnection,  new_rustacean: NewRustacean) -> QueryResult<Rustacean>{
        diesel::insert_into(rustacean::table)
            .vallues(new_rustacean)
            .get_result(c)
            .await
    }
    pub async fn update(c: &mut AsyncPgConnection, id: i32, rustacean: Rustacean) -> QueryResult<Rustacean>{
        diesel::update(rustacean::table.find(id))
            .set((
                rustacean::name.eq(rustacean.name),
                rustacean::email.eq(rustacean.email)
            ))
            .get_result(c)
            .await
    }
    pub async fn delete(c: &mut AsyncPgConnection, id: i32) -> QueryResult<usize>{
        diesel::delete(rustacean::table.find(id)).execute(c).await
    }
}
